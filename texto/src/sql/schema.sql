-- src/sql/schema.sql
--
-- Schema de persistencia para el sistema de texto (Fase 2).
-- Estas tablas reemplazan codebook.json, idf.json, metadata.json
-- y doc_norms.json como fuente de verdad para el backend de
-- produccion (search.py).
--
-- NOTA: esto es independiente del experimento de Fase 3, donde
-- se compara tu indice invertido (SPIMI, archivo final_index.idx)
-- contra un indice GIN nativo de Postgres. La tabla "metadata"
-- de aqui abajo es la que luego se usa para crear ese indice GIN.
