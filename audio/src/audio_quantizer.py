...
class AudioQuantizer:
    def __init__(self, codebook_path=CODEBOOK_FILE_PATH):
        try:
            # Memory Mapping (Lectura bajo demanda del Codebook)
            self.codebook = np.load(codebook_path, mmap_mode='r')
            self.num_words = self.codebook.shape[0]
            print(f"Cuantizador inicializado con {self.num_words} palabras acústicas (mmap).")
